// A simple SDK client for AutoSSL.
// API documentation: https://www.auto-ssl.cn/docs/OpenAPI.html
package autossl
...
func NewClient(optFns ...OptionsFunc) (*Client, error) {
	...
	httper := resty.New().
		SetBaseURL("https://api.auto-ssl.cn").
...
func (c *Client) doRequestWithResult(req *resty.Request, res sdkResponse) (*resty.Response, error) {
	...
	if len(resp.Body()) != 0 {
		if err := json.Unmarshal(resp.Body(), &res); err != nil {
			...
			if rCode := res.GetCode(); rCode != 0 && rCode/100 != 2 {
				return resp, fmt.Errorf("sdkerr: api error: code='%d', status='%s', message='%s'", rCode, res.GetStatus(), res.GetMessage())
			}
		}
	}
